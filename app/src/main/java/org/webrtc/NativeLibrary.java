...
class NativeLibrary {
    ...
    static class DefaultLoader implements NativeLibraryLoader {
        ...
        public boolean load(String name) {
            Logging.d(TAG, "Loading library start: " + name
                    + " classLoader:" + getClass().getClassLoader().hashCode());
            try {
                System.loadLibrary(name);
            } catch (Throwable error) {
                Logging.w(TAG, "Failed to load native library: " + error);
                return false;
            }
            Logging.d(TAG, "Loading library end: " + name);
