...
class WebRtcAudioTrack {
    ...
    private final LeRTCAudioListener leRTCAudioListener;
    ...
    private class AudioTrackThread extends Thread {
        ...
        public void run() {
            ...
            while (keepAlive) {
                ...
                //Logging.w(TAG, "AudioTrack keepAlive " + keepAlive);
                ...
                if (count++ % 1000 == 0) {
                    Logging.w(TAG, "AudioTrack.write size ******* " + speakerMute + " / "
                            + sizeInBytes + " / " + byteBuffer.remaining() + " / " + byteBuffer.limit() + " / " + byteBuffer.position());
                }
                if (speakerMute) {
                    byteBuffer.clear();
                ...
                byteBuffer.rewind();
                if (count % 1000 == 0) {
                    Logging.w(TAG, "AudioTrack.write cost:" + (System.currentTimeMillis() - start));
                }
    ...
    WebRtcAudioTrack(Context context, AudioManager audioManager) {
        this(context, audioManager, null /* audioAttributes */, null /* errorCallback */,
                null /* stateCallback */, false /* useLowLatency */, true /* enableVolumeLogger */, false, null);
    }
    ...
    WebRtcAudioTrack(Context context, AudioManager audioManager,
                     ...
                     boolean enableVolumeLogger, boolean play, LeRTCAudioListener listener) {
        ...
        this.play = play;
        this.leRTCAudioListener = listener;
    ...
    private int initPlayout(int sampleRate, int channels, double bufferSizeFactor) {
        ...
        final int minBufferSizeInBytes = (int) (AudioTrack.getMinBufferSize(sampleRate, channelConfig,
                ...
                * bufferSizeFactor);
        Logging.d(TAG, "minBufferSizeInBytes: " + minBufferSizeInBytes + " / " + byteBuffer.capacity());
        ...
        try {
            ...
            if (useLowLatency && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                audioTrack = createAudioTrackOnOreoOrHigher(
                        sampleRate, channelConfig, minBufferSizeInBytes, audioAttributes, play);
            } else {
            ...
            audioTrack.setRTCListener(leRTCAudioListener);
    ...
    // refined volume or routing decisions.
    private static IAudioTrack createAudioTrackBeforeOreo(int sampleRateInHz, int channelConfig,
                                                          int bufferSizeInBytes, @Nullable AudioAttributes overrideAttributes, boolean play) {
        Logging.d(TAG, "createAudioTrackBeforeOreo");
        logNativeOutputSampleRate(sampleRateInHz);

        if (play) {
            // Create an audio track where the audio usage is for VoIP and the content type is speech.
            return new AndroidAudioTrack(getAudioAttributes(overrideAttributes),
            ...
            return new VirtualAudioTrack(new AudioFormat.Builder()
                    .setEncoding(AudioFormat.ENCODING_PCM_16BIT)
                    .setSampleRate(sampleRateInHz)
                    .setChannelMask(channelConfig)
                    .build(),
    ...
    private static IAudioTrack createAudioTrackOnOreoOrHigher(int sampleRateInHz, int channelConfig,
                                                              int bufferSizeInBytes, @Nullable AudioAttributes overrideAttributes, boolean play) {
        ...
        if (play) {
            // Create an audio track where the audio usage is for VoIP and the content type is speech.
//            return new AndroidAudioTrack.Builder()
...
//                    .build();
            // TODO setPerformanceMode(AudioTrack.PERFORMANCE_MODE_LOW_LATENCY)
            return new AndroidAudioTrack(getAudioAttributes(overrideAttributes),
                    new AudioFormat.Builder()
                            .setEncoding(AudioFormat.ENCODING_PCM_16BIT)
                            .setSampleRate(sampleRateInHz)
